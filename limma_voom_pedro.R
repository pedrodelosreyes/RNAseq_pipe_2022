...
## La replica 1 se va en todas las líneas. Hago BATCH correction
...
boxplot(log2(sample.counts+1),col=met.brewer(n=9, name="Hiroshige"),ylab="log2(counts + 1)",cex.lab=1.5)
...
group <- as.factor(genotype) #Create a new variable “group” that combines cultivar and time
...
ggplot(data = as.data.frame(mean.counts.2.3), aes(x=x35SCO, y=Col0)) + geom_point() + theme_minimal()
...
p <- ggplot(data = as.data.frame(mean.counts.2.3), aes(x=x35SCO, y=Col0, col=diffexpressed)) + geom_point() + theme_minimal() +scale_color_manual(values=mycolors)
...
ggplot(data = as.data.frame(mean.counts.1.2), aes(x=co10, y=Col0)) + geom_point() + theme_minimal()
...
p <- ggplot(data = as.data.frame(mean.counts.1.2), aes(x=co10, y=Col0, col=diffexpressed)) + geom_point() + theme_minimal() +scale_color_manual(values=mycolors)
p + ggtitle("Scatterplot") + xlab("log2(CPM) co-10") + ylab("log2(CPM) Col-0")
...
x35SCO.wt$diffexpressed[x35SCO.wt$logFC > log2(2) & x35SCO.wt$adj.P.Val < 0.05] <- "UP"
# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
x35SCO.wt$diffexpressed[x35SCO.wt$logFC < -log2(2) & x35SCO.wt$adj.P.Val < 0.05] <- "DOWN"
...
volcano + geom_vline(xintercept=c(-log2(1.2), log2(1.2)), col="red") +
  geom_hline(yintercept=-log10(0.05), col="red")
...
co10.wt$diffexpressed <- "NO"
# if log2Foldchange > 0.6 and pvalue < 0.05, set as "UP" 
co10.wt$diffexpressed[co10.wt$logFC > log2(1.5) & co10.wt$P.Value < 0.05] <- "UP"
# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
co10.wt$diffexpressed[co10.wt$logFC < -log2(1.5) & co10.wt$P.Value < 0.05] <- "DOWN"
...
volcano + geom_vline(xintercept=c(-log2(1.5), log2(1.5)), col="red") +
  geom_hline(yintercept=-log10(0.05), col="red") + 
  scale_color_manual(values=mycolors)
...
# tiff("images/heatmap_degs.tiff", height = 4, width = 6, 
...
df.pca.data <- data.frame(PC1 = pca.data$x[,1], PC2 = pca.data$x[,2], sample = colnames(voom.counts), 
                          condition = c(rep("co10",3), rep("Col0",3), rep("35SCO",3)))
head(df.pca.data)
...
# tiff("PCA.tiff", height = 4, width = 6,
...
# Here you can choose the gene to plot
gene.to.plot <- "AT5G24470"
gene.alias <- "PRR5"
...
# gene.expression <- data.frame(expression=log2(cpm[gene.to.plot,]+1),
                              ...
                              genotype= c("co10", "co10", "co10",
                                          "Col-0", "Col-0", "Col-0",
                                          "35SCO", "35SCO", "35SCO"))

sd.35sco <- sd(subset(gene.expression, genotype == "35SCO")$expression)
sd.col0 <- sd(subset(gene.expression, genotype == "Col-0")$expression)
sd.co10 <- sd(subset(gene.expression, genotype == "co10")$expression)

mean.35sco <- mean(subset(gene.expression, genotype == "35SCO")$expression)
mean.col0 <- mean(subset(gene.expression, genotype == "Col-0")$expression)
mean.co10 <- mean(subset(gene.expression, genotype == "co10")$expression)

gene.mean <- data.frame(mean=c(mean.35sco, mean.col0, mean.co10),
                        sd=c(sd.35sco, sd.col0, sd.co10 ),
                        genotype=c("35SCO", "Col-0", "co10"))
head(gene.mean)
...
stripchart(y$E["AT5G15840",]~group)

gene.to.plot <- "AT5G15840"
gene.alias <- "CO"
nice.col <- wes_palette(6,name="BottleRocket1")
...
colnames(gene.expression) <- c("co10_1", "co10_2", "co10_3",
                               "Col0_1","Col0_2","Col0_3",
                               "35SCO_1", "35SCO_2", "35SCO_3")
...
zero.genes <- subset(as.data.frame(gene.expression), Col0_1 == 0 & Col0_2 == 0 & Col0_3 ==0)
nrow(zero.genes)
zero.genes[3000:3050,]
zero.genes[2000:2050,]
zero.genes[5000:5100,]
